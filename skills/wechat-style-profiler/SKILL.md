...
4. 文章路径（可选）
5. 发布反馈（可选）：如果已有“明显好于前几篇”的已发布样本，需同时提供 1-2 篇近期普通样本做对照
...
4. 如果提供了发布反馈，先做一次 `结果反馈反向分析`：比较高反馈样本与普通样本，提炼 3-5 条有效差异，标记哪些是可复用杠杆，哪些只是一次性偶然因素。
5. 按 14 维框架做深度分析（见 `references/style-14d-framework.md`）。
6. 单独总结 `标点符号偏好`，明确破折号、引号、问号、感叹号、省略号等符号的频率、功能和禁用场景。
7. 单独总结 `分块习惯`，明确大标题、小标题、数字分节、层级深度和分块密度。
8. 单独总结 `段落配方`，明确不同场景下的段落长度、句数比例和一句成段的触发条件。
9. 单独总结 `叙述方法体系`，明确冲突、递进、转折、收束的写法。
10. 单独总结 `内容推进方式`，明确作者一般如何把内容往前推，如何从现象推进到观点、从案例推进到判断。
11. 提取 `高频表达词组库`，按场景分类（转场、判断、自嘲、情绪、拉近距离、口头禅），从参考文章中直接提取。
12. 产出初版风格画像草稿并进入用户校准循环。
13. 从参考文章提取 3-5 段 Few-shot 示例。
14. 生成情绪曲线与触发词清单。
15. 总结起承转合模板库。
16. 输出最终风格画像到固定文件，并给出写稿和标题技能的调用说明。
...
7. `叙述方法体系`
8. `内容推进方式`
9. `高频表达词组库`（按转场、判断、自嘲、情绪、拉近距离、口头禅分类）
10. `Few-shot 示例段落`
11. `情绪曲线分析`
12. `起承转合模板`
13. `可复用 Prompt Block`
...
- 不允许把“单篇数据更好”直接等同于“全部风格都该改写”；必须提炼成可复用的写法差异。
- 不允许跳过标点符号偏好分析。
- 不允许跳过分块习惯分析。
- 不允许跳过段落配方和叙述方法体系。
- 不允许跳过内容推进方式分析。
